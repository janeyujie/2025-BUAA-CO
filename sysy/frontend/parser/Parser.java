...
import sysy.error.SyntaxError;
import sysy.error.Error;
import sysy.frontend.lexer.Token;
import sysy.frontend.lexer.TokenType;
...
public class Parser {
    ...
    public List<Error> getErrors() {
        return new ArrayList<>(new ArrayList<Error>(errors));
    }
    ...
    private ConstDef parseConstDef() {
        ...
        node.lineNumber = currentToken().getLineno(); //constdef行数为ident所在行数
        // 匹配是否为标识符
        expect(TokenType.IDENFR, "ident_error", false);

        if (currentToken().getType() == TokenType.LBRACK) {
            consumeToken();
            node.arrayDim = parseConstExp();
    ...
    private VarDef parseVarDef() {
        ...
        node.identName = currentToken().getContent();
        node.lineNumber = currentToken().getLineno();
        expect(TokenType.IDENFR, "ident_error", false);
    ...
    private FuncFParam parseFuncFParam() {
        ...
        param.lineNumber = currentToken().getLineno();
        expect(TokenType.IDENFR, "ident_error", false);
        if (currentToken().getType() == TokenType.LBRACK) {
            consumeToken();
            param.isArray = true;
    ...
    private StmtNode parseStmt() {
        ...
        switch (currentToken().getType()) {
            ...
            case RETURNTK:
                ...
                returnnode.lineNumber = currentToken().getLineno();
                consumeToken();
                if (currentToken().getType() != TokenType.SEMICN) {
            ...
            case PRINTFTK:
                ...
                printfnode.lineNumber = currentToken().getLineno();
                consumeToken();
...
//                }
                ...
                if (currentToken().getType() == TokenType.ASSIGN) {
                    ...
                    assignnode.rValue = parseExp();
                    assignnode.lineNumber = lvalNode.lineNumber;
                    expect(TokenType.SEMICN, "i", true);
                    node = assignnode;
                } else {
                    // 如果失败的话就要回滚
    ...
    private ExprNode parseUnaryExp() {
        ...
        if (currentToken().getType() == TokenType.PLUS
            ...
            // Ident '(' [...] ')'情况, 表明调用了一个函数
            FuncCallExp node = new FuncCallExp();
            node.funcName = currentToken().getContent();
            node.lineNumber = currentToken().getLineno();
            consumeToken();  // ident
            consumeToken();  // (
